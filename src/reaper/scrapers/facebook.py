...
from reaper.parsers import GroupParser, PhotoParser, PostParser, ProfileParser, ReelParser, VideoParser
...
class FacebookScraper(BaseScraper):
    ...
    async def run(self) -> dict[str, Any]:
        ...
        if auth.requires_auth:
            logger.warning("Autenticación requerida | reason=%s | url=%s", auth.reason, final_url)
        ...
        logger.info("Parser seleccionado | parser=%s | url_final=%s", parser_name, final_url)

        if not primary_class:
            logger.warning("Sin parser disponible | parser=%s | url=%s", parser_name, final_url)
            return self._error_result(
        ...
        if result.get("raw_data_available"):
            logger.info("Extracción completada | parser=%s | url=%s", parser_name, final_url)
    ...
    async def _fetch(self, override_url: str | None = None):
    ...
    def _enrich_with_traffic(self, result: dict[str, Any], fetch_result) -> dict[str, Any]:
    ...
    def _merge_feeds(
        ...
        for reel_item in reel_feed:
            ...
            if reel_attachments:
                ...
                for field in _FEED_ATTACHMENT_INJECT_FROM_VIDEO_ITEM:
                    if field not in att:
                        source_field = (
                            "post_play_count" if field == "video_post_view_count" else field
                        )
        ...
        video_only = [item for item in video_feed if item.get("id") not in matched_video_ids]
